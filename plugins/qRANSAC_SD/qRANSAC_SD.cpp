...
qRansacSD::qRansacSD(QObject* parent /*=0*/) : QObject(parent), m_action(0) {}
...
static MiscLib::Vector<
    std::pair<MiscLib::RefCountPtr<PrimitiveShape>, size_t> >*
...
void qRansacSD::doAction() {
    ...
    if (!ent || !ent->isA(CC_TYPES::POINT_CLOUD)) {
        m_app->dispToConsole("Select a real point cloud!",
                             ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }
    ...
    rsdDlg.epsilonDoubleSpinBox->setValue(
        .005f * scale);  // set distance threshold to 0.5% of bounding box width
    rsdDlg.bitmapEpsilonDoubleSpinBox->setValue(
        .01f * scale);  // set bitmap resolution (= sampling resolution) to 1%
                        // of bounding box width
    ...
    // for parameters persistence
    {
        ...
        // consistency check
        {
            ...
            if (primCount == 0) {
                m_app->dispToConsole("No primitive type selected!",
                                     ccMainAppInterface::ERR_CONSOLE_MESSAGE);
                return;
            }
        }
    ...
    {
        ransacOptions.m_epsilon = static_cast<float>(
            rsdDlg.epsilonDoubleSpinBox->value() *
            3.0);  // internally this threshold is multiplied by 3!
        ...
        ransacOptions.m_normalThresh = static_cast<float>(
            cos(rsdDlg.maxNormDevAngleSpinBox->value() * CC_DEG_TO_RAD));
    ...
    if (!hasNorms) {
        QProgressDialog pDlg("Computing normals (please wait)", QString(), 0, 0,
                             m_app->getMainWindow());
        ...
        if (pc->reserveTheNormsTable()) {
            ...
            m_app->dispToConsole("Not enough memory to compute normals!",
                                 ccMainAppInterface::ERR_CONSOLE_MESSAGE);
            return;
        }
    }

    // set which primitives are to be detected by adding the respective
    // constructors
    ...
    // primitive (the support)
    // the points belonging to the first shape (shapes[0]) have been sorted to
    // the end of pc,
    // i.e. into the range [ pc.size() - shapes[0].second, pc.size() )
    // the points of shape i are found in the range
    // [ pc.size() - \sum_{j=0..i} shapes[j].second, pc.size() - \sum_{j=0..i-1}
    // shapes[j].second )

    {
        // progress dialog (Qtconcurrent::run can't be canceled!)
        QProgressDialog pDlg("Operation in progress (please wait)", QString(),
                             0, 0, m_app->getMainWindow());
        pDlg.setWindowTitle("Ransac Shape Detection");
        pDlg.show();
        QApplication::processEvents();
...
//}
    ...
    if (remaining == count) {
        m_app->dispToConsole("Segmentation failed...",
                             ccMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }
    ...
    if (shapes.size() > 0) {
        ...
        for (MiscLib::Vector<DetectedShape>::const_iterator it = shapes.begin();
            ...
            if (shapePointsCount > count) {
                m_app->dispToConsole("Inconsistent result!",
                                     ccMainAppInterface::ERR_CONSOLE_MESSAGE);
                break;
            }
            ...
            for (unsigned j = 0; j < shapePointsCount; ++j) {
                pcShape->addPoint(
                    CCVector3::fromArray(cloud[count - 1 - j].pos));
                if (saveNormals)
                    pcShape->addNorm(
                        CCVector3::fromArray(cloud[count - 1 - j].normal));
            }
            ...
            switch (shape->Identifier()) {
                ...
                case 1:  // sphere
                {
                    ...
                    pcShape->setName(
                        QString("Sphere (r=%1)").arg(radius, 0, 'f'));
                ...
                case 2:  // cylinder
                {
                    ...
                    pcShape->setName(QString("Cylinder (r=%1/h=%2)")
                                         .arg(r, 0, 'f')
                                         .arg(h, 0, 'f'));
                ...
                case 3:  // cone
                {
                    ...
                    minHeight = maxHeight =
                        cone->Internal().Height(cloud[0].pos);
                    ...
                    pcShape->setName(QString("Cone (alpha=%1/h=%2)")
                                         .arg(alpha, 0, 'f')
                    ...
                    // let's build the cone primitive
                    {
                        ...
                        // we build the transformation matrix from these
                        // vecctors
                ...
                case 4:  // torus
                {
                    ...
                    if (torus->Internal().IsAppleShaped()) {
                        m_app->dispToConsole(
                            "[qRansacSD] Apple-shaped torus are not handled by "
                            "CloudCompare!",
                        ...
                        pcShape->setName(QString("Torus (r=%1/R=%2)")
                                             .arg(minRadius, 0, 'f')
                                             .arg(maxRadius, 0, 'f'));
            ...
            if (!group)
                group = new ccHObject(
                    QString("Ransac Detected Shapes (%1)").arg(ent->getName()));
        ...
        if (group) {
            ...
            pc->setEnabled(false);
            m_app->dispToConsole(
                "[qRansacSD] Input cloud has been automtically hidden!",
                ccMainAppInterface::WRN_CONSOLE_MESSAGE);
